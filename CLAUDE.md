...
### Files created
`web/src/utils/folderPath.ts`, `web/public/images/Icon.png` (browser tab favicon), `api/Services/CsvParser.cs`

### Files modified
`api/Controllers/{AuthController,DocumentsController,UsersController,GroupsController,ApprovalsController,BaseController}.cs`, `api/Services/{AuditService,NotificationService,AccessOverrideService}.cs`, `web/index.html`, `web/src/App.tsx`, `web/src/utils/api.ts`, `web/src/components/pages/{Documents,Search,Login}.tsx`, `web/src/components/custom/{DocumentPreview,DocumentList,LibraryMenus,PdfJsViewer,UserManagement,GroupManagement,FolderTree}.tsx`, `web/src/components/layout/MainLayout.tsx`, `web/src/hooks/useAllDmsDocuments.ts`
...
- No data-modifying migration was part of this session — every backend item (4, 22, 23, 24, 28) is a pure code change (a conditional check, a field assignment inside an existing login flow, a string concatenation, two new endpoints on existing tables, and a new notification/email code path over existing tables), so no `infra/db/init/` script needed to be run against the live Ubuntu database.
- None of this session's C# changes (`AuthController.cs`, `UsersController.cs`, `GroupsController.cs`, `CsvParser.cs`, `NotificationService.cs`, `AccessOverrideService.cs`, `ApprovalsController.cs`, `BaseController.cs`) have been compiled locally — this environment has no .NET SDK/build tool available, same limitation every prior session's Verification section already notes for backend changes made without a live container to rebuild against. All reviewed carefully by hand (brace balance, existing `using static AuditActions`/`using DMS.Api.Services` already in scope, `DevSystemAdminId` pattern copied from `FoldersController.cs`'s own established use of the same fixed GUID, `GroupsController`'s new import method reusing its own existing `GetDescendantGroupIdsAsync` private helper directly since it's declared in the same class, item 28's new `NotificationService` constructor param matching the existing DI registrations for `EmailService`/`AccessOverrideService` in `Program.cs`) but `docker compose build api` on the Ubuntu host is the first real compile any of this code will see.
...
- Item 28's email delivery has not been verified against a real, live document-workflow stage transition (QA accept, Manager approve, etc.) — worth a real end-to-end test (submit a throwaway document through QA → Manager → Final Release with a real configured mailer) once deployed, to confirm the "View Document" link actually opens the right preview and that the access-control filtering doesn't accidentally suppress a legitimate reviewer's notification.

### Known follow-ups
- Persisted PPTX preview bug from earlier sessions remains open.
- None of this session's fixes have been confirmed against the live Ubuntu stack yet — that verification now happens on the user's side after pulling and rebuilding, not in this session.

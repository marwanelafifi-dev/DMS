...
**Status:** Session 40 — fixed a real client-side download-substitution bug (previewing an Office document cached the server-generated PDF over the original file's download path, so downloading a previewed `.doc`/`.xlsx`/`.pptx` silently handed back PDF/text bytes under the original extension — verified against the live MinIO-stored bytes, which are genuinely correct OLE2/Office originals; the bug was 100% client-side), plus a batch of Document Library UI requests (subfolder rows inside the file table, click-folder-name-to-expand, a resizable/persisted folder-pane divider, a collapsible desktop sidebar icon rail, a tag filter) and a new File/Folder Permission override action (View Metadata History) covering the KnowledgeTree legacy-archive button. **Known follow-up (unchanged):** a reopened PPTX document's preview loses its styled slide view and falls back to plain extracted text (see the two pre-existing failing tests in `Documents.test.tsx`).

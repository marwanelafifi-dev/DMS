...
### Design change: sticky document context (two intermediate bug reports, superseded by an explicit simpler policy)

- After confirming the Doc ID fix above worked live (asking "SWS-25120007" correctly answered about "Backup Process-Techniques"), two follow-up questions on the same live conversation each broke document continuity in a different way:
  1. "let me know the kpis in the document?" (a pronoun reference, "the document") answered from a completely unrelated document with a longer title — the pronoun/history fallback (from Session 64) reused `FindExplicitDocument`, whose tie-break picks the **longest** matching title, not the most-recently-discussed one.
  2. First fix attempt: added `FindMostRecentlyMentionedDocument(text, documents)` (last raw-text position wins, not string length) for that fallback. Deployed and about to be re-tested when a second, different-shaped question — "let me know the KPIS" with **no pronoun at all** — again resolved to an unrelated document, since the regex gate (`it|that file|...|the document`) simply never matches phrasing with no reference cue.
  3. Second fix attempt: extended `AnalyzeQueryAsync` with a `followUpDocument` field so the LLM itself would judge, per question, whether it's implicitly continuing the previously-discussed document. Written but never deployed.
- The user then stated the actual desired policy directly: **stay on the same document for every later question by default, until the user explicitly asks about a different document or asks to change the source.** This is simpler and more predictable than a per-question LLM judgment call, so the `followUpDocument` LLM mechanism was removed again (never shipped) in favor of one deterministic rule.
- Final design in `Ask()`: after the same-question `FindExplicitDocument` check (an explicit new document mention always wins), a document already active in the conversation stays active for every subsequent question **unless**: (1) a `DocumentContextResetPhrase` regex matches ("different document", "change the source", "forget that file", etc.), or (2) `LooksLikeNonDocumentTopic` — the question matches a clear tasks/calendar/announcements/dashboard trigger word with **no** document-ish wording (`file`, `document`, `kpi`, `detail`, `section`, `policy`, ...) at all, so genuinely unrelated questions ("what tasks are assigned to me") don't get incorrectly forced onto the last-discussed document. When neither escape applies, `FindMostRecentlyMentionedDocument` resolves which accessible document was mentioned latest in the visible conversation text.
- This entirely replaces the pronoun-regex gate from Session 64 and the short-lived `followUpDocument` LLM field — one rule now governs both, is zero-cost (no LLM call needed to decide stickiness), and works identically whether or not an AI provider is configured.

### Real bug found via live testing: sources dropped from conversation history

- Deployed the sticky-context design above and the same "let me know the KPIs" follow-up still resolved to a wrong, unrelated document ("Order Fulfillment Info Session") — even though the immediately preceding answer's full text plainly said `titled "Backup Process-Techniques"`.
- Root cause: the "Authorized sources" list shown in the chat UI is a separate, exact, structured field (`message.sources`) — not part of the model's free-form prose (`message.text`). `AiChatbot.tsx`'s history-building only ever sent `message.text`. The model's own prose happens to restate a title correctly *sometimes*, but that's incidental, not guaranteed — and even when it does, minor punctuation differences (the model wrote "Backup Process-Techniques", but wrote it with a different separator character earlier in the same answer, or Session 65's own citation format uses a slash) can silently defeat an exact substring match server-side.
- Fixed on both ends: `AiChatbot.tsx` now appends the exact source titles (`message.sources`, the same structured data already rendered as clickable "Authorized sources" links) onto the text sent for each assistant turn in history — `[Sources: Backup Process-Techniques]` — so the backend always has the literal, authoritative title to match against, not just whatever the model's prose happened to include. `FindMostRecentlyMentionedDocument` (`AiChatController.cs`) also now normalizes away punctuation/casing differences before comparing title/filename text (`NormalizeForLooseMatch`: strip everything but letters/digits to single spaces, lowercase), so "Backup Process-Techniques" and "Backup Process/Techniques" compare equal — defense in depth beyond the sources fix, in case the model's own prose is ever the only place a title appears in a future exchange.
- A Doc ID mention within the conversation text is still resolved separately, on raw (unnormalized) text via the existing `DocIdLikeToken` regex — kept deliberately un-mixed with the normalized title/filename index space, and short-circuits as an unambiguous win when present, since typing an exact Doc ID again is a much stronger signal than a paraphrased title.

### Real bug found via live testing: hallucinated answer content

- With sticky context now correctly staying on "Backup Process-Techniques" (confirmed via the "Authorized sources" citation), the actual answer to "let me know the KPIs" was still wrong: it invented plausible-sounding metrics — "recovery point objectives (RPO)", "recovery time objectives (RTO)", "backup success rates" — none of which exist anywhere in the real document. The document's real KPI table (Doc Preview, "Measurements" section, page 15) is a single row: **Synology Backup** — percentage of backup attempts completed successfully, measured quarterly.
- Two compounding root causes:
  1. `BuildContext` (`AiChatController.cs`) always sent only the **first 5,000 characters** of a document's extracted text to the model, regardless of document length. This document is 5.92 MB; its KPI table is on page 15 — almost certainly past that fixed cutoff, so the model most likely never received the real table at all.
  2. Once genuinely out of real content, the model filled the gap with plausible, domain-appropriate-sounding invented specifics instead of saying "not available" — a real trust problem for a system whose whole purpose is answering from authorized, verifiable source text (ISO 9001/27001 compliance context).
...
### Investigation: is Docling's table extraction actually the bottleneck?

- Per an explicit request to verify the foundational risk before building more retrieval logic on top of it: fetched the real OCR-indexed content for "Si-Ware - Backup Process Techniques - R5.docx" directly from the OCR service (`GET /api/documents/search?q=Measurement+Frequency`) and inspected it live. **Extraction is clean and accurate** — the real "Measurements" table (`Synology Backup`, its description, formula, and `Quarterly` frequency) converts to well-formed markdown that exactly matches the source document. This rules out Docling table-extraction fidelity as the cause of the earlier hallucination; that was purely a retrieval-window problem, already fixed above.
- A genuinely useful side-finding from the same query: **most procedure documents in this DMS share a near-identical "Measurements" KPI-table template** (same column headers, same "Quarterly" frequency, similar "Percentage of X completed successfully" phrasing) — Security Operational Procedure, Incident Response Plan, Sales & Marketing Procedure, and others each have their own KPI row using this same boilerplate. A bare keyword search for "KPI" alone cannot meaningfully distinguish between them; this is exactly why the sticky-document-context fix above matters so much for follow-up questions, and why a completely fresh, generic broad question with no document-identifying word in it (not this session's failure mode, which was always a follow-up) remains the one case genuine semantic/embedding search would still meaningfully help.
- A second, separate finding from the same query, **not yet addressed**: two indexed documents share a near-identical name — `Si-Ware - Backup Process Techniques - R5.docx` (`cc6e1e9f-5dca-54a7-a221-8755eeb51937`, the real one, with the KPI table) and `Si-Ware-Backup Process - Techniques .docx` (`671445d8-27d4-5d57-8094-ad8273292222`, no measurement content at all). Looks like a stale/duplicate OCR index entry rather than two genuinely distinct DMS documents — worth a dedicated look at some point, but explicitly out of scope for this session's AI-assistant work.

### Quality enhancement: same-name document disambiguation

- Directly motivated by the duplicate-document finding above: asked whether the assistant can tell two same-or-nearly-same-named accessible documents apart when a question references them by name rather than by (unique) Doc ID. Before this fix, `ResolveExplicitDocument`'s predecessor silently picked whichever candidate had the **longest** matching title/filename — the same class of silent-wrong-guess risk fixed for conversation history in the pronoun/sticky-context work earlier in this session, just never closed for the *current* question itself.
- `FindExplicitDocument` was replaced with `ResolveExplicitDocument`, returning a new `ExplicitDocumentMatch(Document, AmbiguousCandidates)`. A Doc ID match still always wins outright and is never ambiguous (Doc IDs are enforced unique across the DMS by a database constraint — Session 26). A filename/title match has no such guarantee: if more than one accessible document's name matches, the resolver deliberately returns **no** single document — the caller must not guess.
- `Ask()` now checks for this ambiguous case immediately after resolution: if triggered, it returns a clarification listing every matching candidate's title, Doc ID, and **full folder path** (reusing the existing `AuditService.ResolveFolderPathAsync` — the same path-resolution logic already used for audit-log readability elsewhere in the app, rather than writing a second one), and asks the user to specify which one by Doc ID — instead of silently answering from whichever candidate happened to have the longer name.

### Real bug found via live testing: stuck sticky context couldn't be overridden by naming a document with different punctuation

- Reported live: after sticky context had (wrongly, from an earlier turn) locked onto "Record Control Procedure", the user tried to redirect back with "Backup Process Techniques this document" — plain spaces, no hyphen. The assistant stayed stuck, repeating the same wrong "I've started indexing Record Control Procedure" answer.
- Root cause: `ResolveExplicitDocument`'s filename/title matching used an exact `Contains(..., OrdinalIgnoreCase)` check — with no tolerance for punctuation/spacing differences. The real document is titled "Backup Process-Techniques" (hyphen); the user typed "Backup Process Techniques" (spaces). The exact-substring check silently failed, so the question was never recognized as the user explicitly naming a different document — it fell through to sticky context instead, which then had nothing to override it with, since the *reason* it should have overridden was the very check that just failed.
- This was the missing half of the loose-matching work already applied to conversation-history resolution (`FindMostRecentlyMentionedDocument`'s `NormalizeForLooseMatch`) — it had never been applied to the *current question's own* resolution, creating exactly this asymmetry: history-based matching tolerated punctuation differences, but the more authoritative "the user is naming a document right now" check didn't.
- Fixed by applying the same `NormalizeForLooseMatch` normalization to `ResolveExplicitDocument`'s filename/title comparison (Doc ID matching is unaffected — it already had its own tolerant `NormalizeDocId` normalization). A question naming a document with different punctuation, spacing, or casing than the stored title now correctly resolves and overrides sticky context, instead of silently failing and leaving the conversation stuck.

### Quality enhancement: word-overlap document disambiguation

- Per explicit request, extended to a case broader than exact-name collisions: "Backup Techniques" and "Backup Process-Techniques" don't collide under the full-title-substring check above at all (neither title is a substring of the other — "Process" sits in the middle), so a question naming one exactly resolves unambiguously today, correctly. But the user wants the assistant to also recognize when a question only shares a **distinctive word** (not a generic filler word) with more than one accessible document's name, and ask which one is meant instead of guessing — for any user, against whatever documents they personally have access to, not just this one pair.
- Added `FindWordOverlapCandidates(text, documents)`: reuses the existing stopword-filtered keyword extraction (`ExtractSearchTerms`, the same one already used for broad-search terms) to find every accessible document whose title or filename shares at least one such word with the question. Returns every match regardless of count — the caller only treats it as ambiguous when more than one comes back; a single shared word with exactly one document isn't a strong enough signal to resolve on its own.
- Deliberately placed as a **last resort** in `Ask()`, checked only when there's no exact name match *and* no active sticky-context document — after the sticky-context step, never before it. Placing it earlier would have broken sticky context itself: an ordinary follow-up in an ongoing document conversation (e.g. "what's the backup schedule" while already discussing a document literally named "Backup...") would otherwise get derailed into a clarification prompt just because it repeats a word from that document's own name.
- The clarification-building logic (candidate title + Doc ID + full folder path) was extracted into a shared `BuildAmbiguousDocumentAnswerAsync` helper so the same-name and word-overlap cases both produce identical, consistent output instead of duplicating the formatting logic.
- **Explicit tradeoff, not yet tested live:** this also fires on ordinary *first-touch* broad questions (no document already active) that happen to share a distinctive word with two or more accessible documents — e.g. asking "what is the backup policy" when two unrelated documents both have "backup" in their name would now prompt for clarification instead of returning a normal synthesized multi-document answer. This is the direct cost of the safety requested; worth judging from real use whether the balance feels right.

### UI: modern, animated chat panel

- Per explicit request ("enhance its UI Appearance", "using animations also", "modern and professional AI Corporate Assistance"): reworked `AiChatbot.tsx`'s visual design while keeping every existing behavior/logic untouched — layout, message flow, and all of the resolution logic above are unaffected.
- Added three CSS keyframe animations to `globals.css`, following the exact same plain-CSS convention (and `prefers-reduced-motion` guard) already established by the app's own `page-fade-in` page-transition animation (Session 38) — no new animation library: `chat-panel-in` (the panel scales/slides in on open), `chat-message-in` (each message fades/slides in — safe to apply unconditionally to every message including old ones, since React never remounts an already-rendered list item in this append-only array, so only the truly new message actually plays the animation), and `chat-typing-dot` (three staggered pulsing dots replacing the old plain spinner for the "Searching your authorized workspace…" loading state).
- Visual refinements: a richer navy gradient header with a soft glow accent and an animated "live" status dot (`animate-ping`), gradient-filled message bubbles (assistant messages gained a left accent bar; user messages a navy-to-blue gradient) instead of flat fills, source citations restyled as elevated hover cards instead of plain underlined links, and tactile `active:scale-95` press feedback on the send button and floating toggle button. Deliberately kept restrained/enterprise rather than a continuous ambient pulse or bouncing toggle button, consistent with this app's own established design ethos (Session 7's explicit rejection of anything reading as "consumer-y").

### New feature: admin-editable AI Assistant system prompts

- Per explicit request ("let me see each one system prompt here and give me the option to edit it"): the two system prompts that shape the assistant's behavior — `GenerateAnswerAsync`'s answer-generation prompt and `AnalyzeQueryAsync`'s query-understanding prompt (added earlier this session) — were hardcoded C# string constants with no way for an admin to see or change them without editing code.
- Added `AiChatPromptsService`, storing both prompts as a single JSON blob under a new reserved `ai_chat_system_prompts` key in the existing `dms_app_settings` table — the same storage mechanism `AiChatSettingsService` already uses for provider configuration (Session 61), just without encryption, since prompt text isn't a secret. A blank field on save resets that one prompt back to its built-in default; loading always falls back to the default text for either prompt if it's unset or the stored payload is malformed.
- Added `AiChatPromptsController` (`GET`/`PUT /api/ai-chat-prompts`), gated on the same `BypassFolderPermissions` ("Full Access acts as admin everywhere") check every other admin-only AI Assistant endpoint already uses. `AppSettingsController`'s existing reserved-key block list (which already protected the provider-config key from the generic settings endpoint) was extended to also protect this new key.
- `AiChatController` now loads both prompts once per request (`promptsService.LoadAsync()`) and passes the relevant one into `GenerateAnswerAsync`/`AnalyzeQueryAsync` as a parameter instead of each method having its own hardcoded constant — behavior is identical to before until an admin actually changes something.
- Frontend: added a new "AI Assistant Prompts" section directly on the existing Admin Panel → API Keys page (`ApiKeysSettings.tsx`), per the explicit "let me see each one system prompt HERE" request — one card per prompt with a live character counter (8,000-char limit, matching the backend's validation), a "Using default"/"Customized" badge, a per-field "Reset to default" button, and its own independent Save action separate from the provider-settings form above it.

### Real bug found via live testing: header title washed out by its own decorative glow

- Reported live: the "DMS AI Assistant" header title (from the UI redesign above) was barely legible, while "Secure workspace assistant" right below it read fine.
- Root cause: the decorative glow (`bg-white/10 blur-2xl`, added purely for visual depth) was large and positioned close enough to the title that it lightened the gradient right behind the text — white text over a lighter patch of background loses contrast even though it still paints on top in DOM order. Fixed by shrinking the glow, tucking it further into the corner away from the title, lowering its opacity (`white/[0.06]`), and adding an explicit `text-white` plus a subtle drop shadow on the title as insurance against this exact class of regression recurring.

### Quality enhancement: attention cue on the floating toggle button

- Per explicit request ("add note to grab attention of users to use it"): the floating toggle button previously gave no signal that the assistant exists or is worth trying. Added a pulsing ring (`animate-ping`) behind the button and a small "Need help? Ask the AI Assistant" label beside it — both shown only until the user opens the assistant for the first time in the session (tracked via a new `hasOpened` state), so the cue never lingers and annoys someone who's already discovered it.

### Real bug found via live testing + new feature: no awareness of Admin Panel data at all

- Reported live by a Full Access user: asking "let me know who are the users of system administrators group" got a nonsense answer — it stayed stuck on whatever document sticky context was already on ("PO") and tried to reindex it, instead of answering about group membership.
- Root cause, two layers: (1) the assistant had **zero context category for Admin Panel data** (Users/Groups/Roles) at all — only documents, tasks, calendar, announcements, and dashboard existed as recognized categories, so a question like this had nowhere real to route to; (2) `LooksLikeNonDocumentTopic` (the sticky-context release check) didn't recognize admin/group/user wording either, so it never broke out of sticky context to even try.
- Per explicit follow-up, scoped to *all* Admin Panel data (not just Groups) for Full Access users specifically (not the weaker `CanViewAdminPanel` flag some non-Full-Access roles can also hold) — the user was explicit that this should key off the Full Access role boundary, the same "Full Access acts as admin everywhere" (`BypassFolderPermissions`) check already used for every other admin-only AI Assistant capability in this session (API Keys, system prompts).
- Deliberately excluded from this pass: Database/backup status and API Keys — these can involve credentials (SMB share passwords, provider keys) that must never flow through free-text AI prose, regardless of the caller's role. Audit Trail was also excluded — its size and table shape don't fit a conversational context window well; a real answer here (beyond this session's scope) would need its own summarized/paginated approach, not a raw dump into the prompt.
- Added a sixth `ChatIntent`/`QueryUnderstanding` category, `AdminInfo`, detected both by keyword (`DetectIntent`: "administrator(s)", "user group(s)", "group members", "members of", "list users", "page access role", etc.) and by the LLM query-understanding step (extended `AiChatPromptsService.DefaultQueryUnderstandingPrompt`'s JSON schema with an `"adminInfo"` boolean). The same keyword set was also added to `LooksLikeNonDocumentTopic` so sticky document context correctly releases for this category too, fixing the root cause of the live bug above.
...
### Real bug found via live testing: neither fix above actually held after a confirmed rebuild+redeploy

- The user explicitly confirmed both `api` and `web` were rebuilt and hard-refreshed after the two fixes above, and both symptoms persisted unchanged — the header title still unreadable, and "IT Group Team members ?" (a straightforward co-occurrence match under the new detection logic) still returned the same generic fallback. Root-caused both properly rather than guessing a fourth CSS tweak.
- **Admin-info regression, real root cause:** the query-understanding system prompt became admin-editable and database-persisted earlier this session (`AiChatPromptsService`, `dms_app_settings`). If anyone had ever opened the new "AI Assistant Prompts" admin page and clicked Save — even without editing anything — the textarea's current contents at that moment get persisted as a "customized" prompt, permanently overriding the compiled-in default from then on. Since the JSON-shape instruction (including the `adminInfo` field) lived *inside* that same editable prompt text, a save that predates this session's addition of that field would freeze the model's instructions on the old shape forever — and rebuilding/redeploying the containers never touches data already sitting in `dms_app_settings`, so no amount of redeploying code alone could ever fix it.
- Fixed structurally, not just for this one field: added `AiChatController.QueryUnderstandingJsonContract`, a code-level constant describing the exact required JSON output shape, now always appended in `AnalyzeQueryAsync` to whatever prompt text is actually in effect (default or customized) rather than living only inside the editable text. `AiChatPromptsService.DefaultQueryUnderstandingPrompt` was simplified to describe only search-term/category *guidance* — the JSON contract itself is no longer something a saved customization (or a future added category) can ever go stale on.
- **Header title, real root cause (previously unidentified):** `globals.css` has a bare `h2 { color: #283a7a; ... }` element-selector rule with no light/dark-mode branch beyond `dark:text-white` — meaning in light mode, this global rule and the component's own `text-white` Tailwind utility class both target the `<h2>`, and something in how Tailwind's generated utility layer interacts with this plain, unlayered rule was letting the global navy color win regardless of which specific override technique (opacity tuning, then full removal of the competing glow, then explicit `text-white` plus a drop shadow) was tried — the exact same underlying bug class already documented once before in this project, for a different pair of elements (Session 7). None of those attempts could have worked, since none of them changed which *rule* wins, only what color the losing side was trying to apply.
- Fixed with an inline `style` attribute on both the header `<h2>` and its `<p>` subtitle instead of relying on any Tailwind color utility class at all — an inline style is applied directly to the element and cannot be beaten by any external stylesheet rule regardless of specificity, source order, or CSS layer interactions, closing this off for good rather than trying a fourth variation of the same losing technique.

### Real bug found via live testing: the "structural" fix above still wasn't enough — moved the classification rules into code too, not just the shape

- After redeploying the fix above, three worded-differently admin-info questions in a row ("let me know who are in the IT Group ?", "i mean the IT Group Users ?", "i mean the IT Group Members ?") — including the third, a textbook-clean match for the keyword co-occurrence rule — all still returned the same generic fallback, never even the explicit "Only a Full Access role can view..." denial. Since that denial is the *only* other possible outcome once `intent.AdminInfo` is true, its complete absence proved `AdminInfo` was never becoming true at all, for any of the three phrasings.
- Real gap in the previous fix: `QueryUnderstandingJsonContract` only specified the JSON **shape** (the field name `adminInfo` itself) — it never told the model **when** to set it true. That guidance had only ever lived in the editable, database-persisted prompt text. So even with the shape now guaranteed, a stale saved prompt from before this category existed gave the model a field to fill in with zero instruction on its meaning — which a model will rationally default to `false` for, every time, since it was never told what "adminInfo" means or when a question qualifies.
- Fixed properly this time: moved the full classification guidance for **all six** categories (not just `adminInfo`) into `QueryUnderstandingJsonContract` itself, always appended in code regardless of what's saved in the database. `AiChatPromptsService.DefaultQueryUnderstandingPrompt` now contains only the one thing that's genuinely safe to leave customizable — guidance on *how to phrase* good search terms — with zero correctness-critical instructions left inside editable, staleable text. A future new category can now only ever need a code change to work correctly; no admin action on a settings page can silently break it again.

### Files modified

- `api/Controllers/AiChatController.cs`
- `api/Controllers/AiChatPromptsController.cs` (new)
- `api/Controllers/AppSettingsController.cs`
- `api/Services/AiChatPromptsService.cs` (new)
- `api/Program.cs`
...
- Frontend: `npx tsc --noEmit` clean — only the same three pre-existing, unrelated baseline errors (`NotificationsBell.tsx`, `Dashboard.tsx`, `officeParser.test.ts`) — after every round of changes in this session, including the UI redesign, the new prompts admin page, and the admin-info feature (backend-only, no frontend change needed).
- Backend: manual code review only — brace/paren counts balanced across every touched file (`AiChatController.cs` 184/184 and 612/612 in the final state; `AiChatPromptsController.cs` 10/10 and 29/29; `AiChatPromptsService.cs` 14/14 and 25/25; `AppSettingsController.cs` 22/22 and 34/34); no local `dotnet build` available on this workstation.
- The Doc ID resolution fix was confirmed working live by the user on Ubuntu (asking "SWS-25120007" now correctly resolves to and answers about "Backup Process-Techniques", the exact scenario from the original bug report). Nothing from the sticky-context work onward — including the excerpt-windowing/anti-hallucination fix, multi-chunk retrieval, the disambiguation fixes, the UI redesign, the prompts admin page, or the new admin-info feature — has been rebuilt or re-tested live yet.
...
- Also worth testing: an unrelated follow-up afterward (e.g. "what tasks are assigned to me") should not get forced onto that document; naming a different accessible document, or a document sharing only one distinctive word with another (e.g. "Backup Techniques" vs. "Backup Process-Techniques"), should correctly prompt for clarification or switch away as appropriate; asking about a genuinely-missing detail should get an honest "not available" instead of a plausible-sounding invention; the AI Assistant button/panel should render with the new gradient header, animated entrance, and typing-dot loading indicator; Admin Panel → API Keys should show the new "AI Assistant Prompts" section with both prompts editable, saving, and resetting to default correctly; and, specifically, asking the exact question that surfaced the admin-info bug ("who are the users of system administrators group") as a Full Access user should now return a real list of that group's members, while the same question from a non-Full-Access user should get the explicit "Only a Full Access role can view..." denial instead of either a wrong document answer or silence. Check `docker compose logs api` for `"AI provider {Provider} failed"` / `"Could not parse AI query-understanding response"` warnings that would mean the query-understanding step is silently falling back to the old heuristics.
...
- Only the current file owner or a global Full Access user may change that file's owner. Folder ownership, Manager status, and folder-level Admin grants do not authorize file-owner reassignment.
...
- File-owner reassignment checks the document's current `owner_id` directly and permits the global `BypassFolderPermissions` Full Access override; no folder position or ordinary folder permission is sufficient. Folder-owner reassignment remains Full Access-only.
...
## Session 48 (2026-08-27) — Final Ownership Boundary and Docker Preview Fonts

**Status:** Complete. No database migration is required.

### Final ownership boundary

- File ownership can be reassigned only by the file's current owner or a global Full Access user (`BypassFolderPermissions`).
- Being the containing folder's owner, a selected folder manager, a Manager-role user, or holding a folder-level Admin grant does not authorize changing a file owner.
- This exact check is enforced in both Edit Document and Upload New Version, including crafted API requests; the UI owner selector uses the same document-specific capability response.
- Folder ownership remains stricter: only a global Full Access/system administrator may transfer a folder. The current folder owner cannot transfer the folder without Full Access.

### Dockerized PPTX font fidelity

- The PPTX preview is generated by headless LibreOffice Impress in `ocr-rag`, then displayed as PDF. Layout drift seen against local Microsoft PowerPoint was traced to missing source fonts and Linux font substitution, not file corruption.
- `docker-compose.yml` now mounts the deployment-local `./infra/fonts` directory read-only at `/usr/local/share/fonts/dms` and runs `fc-cache -f` before starting Uvicorn.
- The default `DMS_PREVIEW_CACHE_FORMAT_VERSION` is now `libreoffice-pdf-v2`, invalidating PDFs cached before production fonts were installed.
- `infra/fonts/.gitignore` prevents `.ttf`/`.otf` binaries—especially licensed Microsoft fonts—from being committed. `infra/fonts/README.md` documents deployment and verification.
- The Ubuntu deployment currently supplies Arial, Arial Narrow, Calibri, and Poppins variants in `/opt/dms/infra/fonts`. Font licensing remains the deployer's responsibility.
...
- `web/src/components/custom/UploadNewVersionModal.tsx`
- `docker-compose.yml`
- `infra/fonts/.gitignore`
- `infra/fonts/README.md`

### Deployment verification

- Recreate `ocr-rag`, confirm it is healthy, and use `fc-match` for Arial, Calibri, and Poppins.
- Reopen a PPTX after a hard browser refresh; cache version `v2` forces a fresh conversion.
...
**Current Status:** Session 44 — folder-owner and manager permissions were refined; document Copy now creates a clean persisted Draft with no inherited Doc ID/workflow state; and file/folder deletion is recoverable through a Full-Access Recycle Bin with an exact typed `DELETE` safeguard.

---

## Session 44 (2026-08-27) — Permission Refinements, Safe Copy, and Recoverable Deletion

**Status:** Complete in code. Focused Document Library and copy-operation tests pass. Frontend type-checking reaches only the same five pre-existing baseline errors (`unreadCount`, `canEditFiles` ×2, `PendingApprovalItem` ×2). The API was not compiled locally because this checkout has no `dotnet` executable.

### Folder owner and manager permissions

- Folder owners now have a guaranteed permission floor for file deletion and force-unlock.
- A folder-role Manager can edit document metadata.
- Changing the document Owner is separately restricted to the folder owner, Full Access, or another effective Admin folder grant. Ordinary Managers see the Owner selector disabled, and the API rejects direct bypass attempts.

### Safe persisted document copy

- Document Copy is now a real API and MinIO operation instead of a temporary React-state clone.
- A copy receives a new document ID, version ID, stored object, and collision-safe `Copy` filename.
- Every copy starts as Draft and clears `TrackingCode`, `OriginalDocumentId`, Doc-ID presence, checkout state, submission/approval state, and approval rows. Source metadata and file bytes are retained.

### Recycle Bin and deletion safeguard

- Added `087_recycle_bin.sql`, introducing soft-delete metadata and deletion-batch IDs for folders/documents, indexes for deleted records, and active-only folder-name uniqueness.
- EF global query filters exclude deleted folders and documents from ordinary library, search, workflow, notification, and API queries.
- File deletion retains the document, versions, MinIO objects, Doc ID, metadata, permissions, and related workflow records for restoration.
...
Apply migration `infra/db/init/087_recycle_bin.sql` to the production PostgreSQL database before starting the updated API, then rebuild/restart the API and web containers.

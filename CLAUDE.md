...
**Status:** Complete in code. Rebuild `api`; no database migration is required. Verified by manual code review plus balanced brace/paren counts (no .NET SDK/Docker on this Windows workstation) — the Doc ID fix itself was confirmed working live by the user on Ubuntu mid-session; the final sticky-document-context design below has not yet been rebuilt/re-tested live.
...
- Manual code review only — brace/paren counts balanced (140/140, 438/438 in the final state) across the whole file; no local `dotnet build` available on this workstation.
...
- The sticky-document-context design has **not** yet been rebuilt or tested live. Rebuild `api` on Ubuntu (`docker compose build api && docker compose up -d api`), then repeat both failing scenarios from this session — a pronoun follow-up ("what are the kpis in the document?") and a no-pronoun implicit follow-up ("let me know the KPIs") right after an answer about a specific document — and confirm both now stay on that same document. Also test that asking about an unrelated topic afterward (e.g. "what tasks are assigned to me") does **not** get incorrectly forced onto that document, and that naming a different accessible document, or saying something like "let's talk about a different document," correctly switches away from it.

...
- The Document Preview's own metadata header ("Folder" field) had the identical bare-name limitation — it now shows the same full path, via a new optional `folders` prop threaded in from `Documents.tsx`.
...
### Files created
`web/src/utils/folderPath.ts`

### Files modified
`api/Controllers/DocumentsController.cs`, `web/src/App.tsx`, `web/src/components/pages/{Documents,Search}.tsx`, `web/src/components/custom/{DocumentPreview,DocumentList,LibraryMenus,PdfJsViewer,UserManagement,FolderTree}.tsx`, `web/src/components/layout/MainLayout.tsx`, `web/src/hooks/useAllDmsDocuments.ts`

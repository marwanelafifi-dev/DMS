...
### Files created
`web/src/utils/folderPath.ts`, `web/public/images/Icon.png` (browser tab favicon)

### Files modified
`api/Controllers/{AuthController,DocumentsController}.cs`, `api/Services/AuditService.cs`, `web/index.html`, `web/src/App.tsx`, `web/src/components/pages/{Documents,Search}.tsx`, `web/src/components/custom/{DocumentPreview,DocumentList,LibraryMenus,PdfJsViewer,UserManagement,FolderTree}.tsx`, `web/src/components/layout/MainLayout.tsx`, `web/src/hooks/useAllDmsDocuments.ts`
...
- No data-modifying migration was part of this session — items 4 and 22 are both pure code changes (a conditional check, and a field assignment inside an existing login flow), so no `infra/db/init/` script needed to be run against the live Ubuntu database.
- Item 22's C# change (`AuthController.cs`) has **not** been compiled locally — this environment has no .NET SDK/build tool available, same limitation every prior session's Verification section already notes for backend changes made without a live container to rebuild against. Reviewed carefully by hand (brace balance, existing `using static AuditActions` already in scope, `DevSystemAdminId` pattern copied from `FoldersController.cs`'s own established use of the same fixed GUID) but `docker compose build api` on the Ubuntu host is the first real compile this code will see.
...
- A "bulk import users from Google Workspace" feature was requested at the end of this session but not yet scoped or built — needs a decision on Google Admin SDK / Directory API access (a service account with domain-wide delegation, separate from the existing Sign-In OAuth Client ID) before any code can be written.

...
### Files modified
`api/Controllers/{AuthController,DocumentsController,UsersController,GroupsController,ApprovalsController,BaseController}.cs`, `api/Services/{AuditService,NotificationService,AccessOverrideService,EmailService}.cs`, `web/index.html`, `web/src/App.tsx`, `web/src/utils/api.ts`, `web/src/components/pages/{Documents,Search,Login}.tsx`, `web/src/components/custom/{DocumentPreview,DocumentList,LibraryMenus,PdfJsViewer,UserManagement,GroupManagement,FolderTree}.tsx`, `web/src/components/layout/MainLayout.tsx`, `web/src/hooks/useAllDmsDocuments.ts`
...
- No data-modifying migration was part of this session — every backend item (4, 22, 23, 24, 28, 29) is a pure code change (a conditional check, a field assignment inside an existing login flow, a string concatenation, two new endpoints on existing tables, and new notification/email/audit code paths over existing tables), so no `infra/db/init/` script needed to be run against the live Ubuntu database.
- None of this session's C# changes (`AuthController.cs`, `UsersController.cs`, `GroupsController.cs`, `CsvParser.cs`, `NotificationService.cs`, `AccessOverrideService.cs`, `ApprovalsController.cs`, `BaseController.cs`, `EmailService.cs`, `AuditService.cs`) have been compiled *locally* — this environment has no .NET SDK/build tool available, same limitation every prior session's Verification section already notes for backend changes made without a live container to rebuild against — but item 28/29's core email-delivery path is now confirmed compiling and working correctly on the real Ubuntu host, per the user's own live test.
...
- Item 29's new `NOTIFICATION_SENT` Audit Trail logging and the folder-tree collapsed-by-default change (item 30) have not yet been verified live — the email-delivery path they build on top of (item 28) has been, but these two specific additions were made after that live confirmation and are still awaiting their own rebuild/deploy.

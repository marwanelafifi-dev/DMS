...
**Status:** Session 39 — added a real API Gateway (Traefik) in front of web+api as pure, additive infrastructure for the planned vCenter/microcloud deployment (existing direct ports untouched; local Docker-Desktop-for-Windows has a known socket-provider quirk that blocks Traefik's container auto-discovery specifically in that environment — expected to work unmodified on the real Ubuntu/vCenter target, not yet verified there). Also started the actual KnowledgeTree migration work as a sequence of strictly read-only, reproducible scripts under `migration/scripts/` (metadata extraction → preflight → physical-file validation), each backed by real numbers pulled directly from the legacy MySQL dump and the delivered `blobs/`/`reconstructed/` trees — no PostgreSQL/MinIO/new-DMS write has happened at any point. **Known follow-up (unchanged):** a reopened PPTX document's preview loses its styled slide view and falls back to plain extracted text (see the two pre-existing failing tests in `Documents.test.tsx`).

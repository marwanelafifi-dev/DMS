...
**Status:** Session 38 — a navigation/perf-focused session that started as "add a page transition animation" and turned into a full-system scalability investigation backed by real load testing (not just code review), plus three concrete fixes for what that testing actually found broken: a LibreOffice/OCR conversion pipeline that collapsed completely (100% failure + permanent zombie processes) at just 15 concurrent Word/PowerPoint previews, a document-download path that buffered entire files in API process RAM, and a Document Library table that rendered every row in a folder into the DOM at once with no ceiling. Also: a real architectural fix to `MainLayout`/`App.tsx` so the Sidebar/Navbar mount once per session instead of being torn down and rebuilt (with all their own data-fetching) on every single navigation, and a full read-only technical-discovery pass (`DMS_MIGRATION_TECHNICAL_PROFILE.md`) for the upcoming KnowledgeTree migration. **Known follow-up (unchanged):** a reopened PPTX document's preview loses its styled slide view and falls back to plain extracted text (see the two pre-existing failing tests in `Documents.test.tsx`).

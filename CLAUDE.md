...
**Status:** Session 37 — another long, screenshot-driven session, mostly in the File/Folder Permission override system: a silently-dropped "Approval Notes" field, a task-attachment preview that showed raw bytes instead of rendering, a real race condition that made a fresh-navigation document preview hang forever, an explicit removal of the task-assignee document-access bypass (replaced with real folder-permission requirements), two real override-resolution gaps (Folder Read not implying File View, File View not implying the Download action the previewer actually depends on), a File Permissions modal that couldn't show or edit a folder-inherited override, and the session's biggest single bug — every document metadata PUT was gated on the wrong override action (Rename instead of Edit) at the middleware layer, silently breaking Edit for anyone who had Edit but not Rename. Also: immediate-send reminders, a new "Delete Reminders" permission flag, a real welcome email on user creation, and full click-through detail views for Announcement notifications. **Known follow-up (unchanged):** a reopened PPTX document's preview loses its styled slide view and falls back to plain extracted text (see the two pre-existing failing tests in `Documents.test.tsx`).

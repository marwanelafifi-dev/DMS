...
**Status:** Complete in code, including one more real bug found live after deploying the sticky-context design (see "Sources dropped from conversation history" below). Rebuild `api` and `web` together — this last fix touches both. No database migration is required. Verified by manual code review plus balanced brace/paren counts (no .NET SDK/Docker on this Windows workstation) — the Doc ID fix itself was confirmed working live by the user on Ubuntu mid-session; the sources-in-history fix has not yet been re-tested live.
...
- Backend: manual code review only — brace/paren counts balanced (141/141, 447/447 in the final state) across the whole file; no local `dotnet build` available on this workstation.
...
- The sticky-document-context design plus the sources-in-history fix have **not** yet been rebuilt or tested live together. Rebuild both `api` and `web` on Ubuntu (`docker compose build api web && docker compose up -d api web`), then repeat every failing scenario from this session in one conversation: a pronoun follow-up ("what are the kpis in the document?"), a no-pronoun implicit follow-up ("let me know the KPIs"), and the exact multi-turn case that just failed (ask about a document, then ask about its KPIs) — confirm all three now stay on the same document. Also test that asking about an unrelated topic afterward (e.g. "what tasks are assigned to me") does **not** get incorrectly forced onto that document, and that naming a different accessible document, or saying something like "let's talk about a different document," correctly switches away from it.
- Also worth testing a paraphrased broad question (words that don't literally appear in the target document, with no document already active in the conversation) to confirm the LLM query-understanding step from earlier in this session is actually improving recall, and checking `docker compose logs api` for `"AI provider {Provider} failed"` / `"Could not parse AI query-understanding response"` warnings that would mean it's silently falling back to the old heuristics.

...
- Backend: manual code review only — brace/paren counts balanced (164/164, 538/538 in the final state) across the whole file; no local `dotnet build` available on this workstation.
- The Doc ID resolution fix was confirmed working live by the user on Ubuntu (asking "SWS-25120007" now correctly resolves to and answers about "Backup Process-Techniques", the exact scenario from the original bug report). Nothing from the sticky-context work onward — including the excerpt-windowing/anti-hallucination fix, multi-chunk retrieval, and the same-name disambiguation fix — has been rebuilt or re-tested live yet.
...
- Also worth testing: an unrelated follow-up afterward (e.g. "what tasks are assigned to me") should not get forced onto that document; naming a different accessible document, or saying "let's talk about a different document," should correctly switch away from it; asking about a genuinely-missing detail should get an honest "not available" instead of a plausible-sounding invention; and asking a question that names the ambiguous "Backup Process Techniques" title (without its Doc ID) should return the new clarification listing both matching documents with their folder paths, not a silent guess. Check `docker compose logs api` for `"AI provider {Provider} failed"` / `"Could not parse AI query-understanding response"` warnings that would mean the query-understanding step is silently falling back to the old heuristics.

...
// The folder's own metadata (Description/Classification), distinct from
// renaming — gated on the separate "Edit" (FolderEdit) permission rather
// than "Rename".
export function EditFolderModal({ folderId, folderName, initialDescription, initialClassification, onClose, onSaved }: EditFolderModalProps) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await apiClient.updateFolderMetadata(folderId, { description, classification });
  ...
  return (
    <ModalOverlay onClose={onClose} className="fixed inset-0 z-[80] flex items-center justify-center bg-black/50 p-4">
      <div className="flex w-full max-w-lg flex-col overflow-hidden rounded-lg bg-white shadow-xl dark:bg-slate-900">
